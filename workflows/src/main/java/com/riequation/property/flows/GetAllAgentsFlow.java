...
@StartableByRPC
public class GetAllAgentsFlow extends FlowLogic<List<AgentState>> {

    @Suspendable
    @Override
    public List<AgentState> call() {
        // Use the ServiceHub directly to access the VaultService
        VaultService vaultService = getServiceHub().getVaultService();

        // We create a query criteria to fetch all unconsumed AgentState's from the vault.
        ...
        Page<AgentState> results = vaultService.queryBy(AgentState.class, queryCriteria, pageSpec);

        // Map the list of StateAndRef to AgentState
